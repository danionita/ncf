 ...
 *      ------          -------         -----------
 *      MPH             1.0             First Issue
 *      MPH             1.1             Fixed a bug with discarding a line for comments</pre><br>
 *
 *-------------------------------------------------------------------------------------------<br>
 *
 *      IMPORT: java.io<br>
...
public class LookAheadStream
{
    ...
    public boolean openFile(String file) throws ParserException
    {
        // GOR try
        // GOR{
	       // GOR buffR = new PushbackReader(new FileReader(new File(file)));
            buffR = new PushbackReader(new StringReader((file)));
    	    unreadme = new char[kMaxBufferSize];
        	bufferPos = 0;
        // GOR}
        // GOR catch(IOException ioex)
        // GOR{
        // GOR    throw new ParserException("Source file could not be opened.");
        // GOR}
