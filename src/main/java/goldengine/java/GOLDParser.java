 ...
 * -------------------------------------------------------------------------------------------<br>
 */
public class GOLDParser implements GPMessageConstants, SymbolTypeConstants,
        ActionConstants, RecordIDConstants, ParseResultConstants {
    
    static Logger log = Logger.getLogger(GOLDParser.class.getName());
    ...
    private Stack pTokens = new Stack(); // The set of tokens for 1. Expecting
                                         // during error, 2. Reduction
    ...
    /*
     * public void write(BufferedWriter buffW, String toWrite, boolean newLine)
     * throws IOException { buffW.write(toWrite, 0, toWrite.length());
     * if(newLine) { buffW.newLine(); } buffW.flush(); }
     */
     ...
     *             The engine parser should deal with a problem with closing the
     *             source file.
     ***************************************************************/
     ...
     * currentToken
     *
     * This method returns the current Token. The current token is the last
     * token read by "retrieveToken".
     * 
     ...
     * popInputToken
     *
     * This method should only be called if there is a lexical error and you
     * need to pop an unexpected token out of the stack.
     * 
     ...
     * pushInputToken
     *
     * This method should only be used if there is a syntax error. It will push
     * a Token onto the top of the stack so that the parser might have a chance
     * of finding an correctly typed token.
     * 
     * @param theToken
     ...
     * getToken
     *
     * If you require access to tokens in the stack before they are placed on
     * the parse tree. Enter an index number and the Token will be returned, if
     * and only if the index number is valid.
     * 
     * @param index
     *            The index number.
     ...
     * currentReduction
     *
     * This method will return the current reduction. This will only happen if
     * the parsing engine has performed a reduction. If it has, then the
     * reduction passed back will be the last one performed.
     * 
    ...
    private boolean loadTables(String fileName) throws ParserException {
        ...
        if (sdb.openFile(fileName)) {
            while (!sdb.done()) {
                ...
                switch (caseSelect) {
                case recordIdParameters: // Name, Version, Author, About,
                                         // Case-Sensitive
                    ...
                    pVariables.setValue("Case Sensitive",
                            "" + caseS.booleanValue());
                    Integer startS = (Integer) sdb.retrieveNext();
                    pVariables.setValue("Start Symbol", "" + startS.intValue());
                    break;

                case recordIdTableCounts: // Symbol, CharacterSet, Rule, DFA,
                                          // LALR
                ...
                case recordIdRules: // #, ID#, Reserved, (Symbol#, ...)
                    ...
                    readRule.setRuleNonTerminal(
                            pSymbolTable.getMember(nA.intValue()));
                ...
                case recordIdDFAStates: // #, Accept?, Accept#, Reserved (Edge
                                        // chars, Target#, Reserved)...
                ...
                case recordIdLRTables: // #, Reserved (Symbol#, Action, Target#,
                                       // Reserved)...
                    ...
                    while (!sdb.retrieveDone()) {
                        ...
                        readLALR.addItem(pSymbolTable.getMember(nA.intValue()),
                                action.intValue(), target.intValue());
            ...
            pVariables.setValue("Start Symbol",
                    pSymbolTable
                            .getMember(Integer.parseInt(
                                    pVariables.getValue("Start Symbol")))
                            .getName());
            ...
            log.error("GOLDParser.loadTables failed " + fileName);
            return false;
        }
    }
     ...
     * loadCompiledGrammar
     *
     * This method will reset the GOLDParser engine before loading a new .cgt
     * file into it.
     * 
     * @param fileName
     ...
     * @throws ParserException
     *             If there was a stream access problem with the file.
     ***************************************************************/
     ...
     * openFile
     *
     * This method will open the source file for reading. It will also reset all
     * information from previous source files.
     * 
     * @param fileName
     ...
     * reset
     *
     * This method will reset the parser engine. It initalises the Error and
     * Type End symbols, and then clears all the stacks of any tokens.
     ***************************************************************/
    public void reset() {
        Token start = new Token();
     ...
     * parse
     *
     * Will parse a token. 1. If the tables are not setup then report
     * GPM_NotLoadedError<br>
     ...
     * a. If there are no tokens on the stack 1) Read one and trap error 2) End
     * function with GPM_TokenRead b. Otherwise, call ParseToken with the top of
     * the stack. 1) If success, then Pop the value 2) Loop if the token was
     * shifted (nothing to report)
     *
     * @return The result of one parse of the source file. The integer could be
     *         one of the constants defined in the interface GPMessageConstants.
     * @throws ParserException
     *             This is thrown if there are any problems reading information
    ...
    public int parse() throws ParserException {
        ...
        if ((pActionTable.size() < 1) | (pDFA.size() < 1)) {
            log.error("GOLDParser.parse " + pActionTable.size() + " "
                    + pDFA.size());
                ...
                if (pInputTokens.size() == 0) // we must read a token
                {
                    ...
                    if (readToken == null) {
                        ...
                        if (readToken.getKind() != symbolTypeWhitespace) {
                            ...
                            if ((pCommentLevel == 0)
                                    & (readToken
                                            .getKind() != symbolTypeCommentLine)
                                    & (readToken
                                            .getKind() != symbolTypeCommentStart)) {
                    ...
                    if (pCommentLevel > 0) // we are in a block comment
                    {
                        ...
                        default: // FINALLY, we can parse the token
                            ...
                            // NEW 12/2001: Now we are using the internal
                            // enumerated constant ParseResult
                            ...
                            case parseResultShift: // A simple shift, we must
                                                   // continue
                                pInputTokens.pop(); // Okay, remove the top
                                                    // token, it is on the stack
                                break;
    ...
    private int parseToken(Token nextToken) {
        ...
        // 1. Makes a SINGLE reduction and pushes a complete Reduction object on
        // the stack
        // 2. Accepts the token and shifts
        // 3. Errors and places the expected symbol indexes in the Tokens list
        // The Token is assumed to be valid and WILL be checked
        // If an action is performed that requires controlt to be returned to
        // the user, the function returns true.
        ...
        LRActionTable lrAct = (LRActionTable) pActionTable
                .elementAt(currentLALR);
        index = lrAct
                .actionIndexForSymbol(nextToken.getPSymbol().getTableIndex());
        ...
        if (index != -1) // Work - shift or reduce
        {
            ...
            case actionReduce:
                // Produce a reduction - remove as many tokens as members in the
                // rule & push a nonterminal token
                ...
                if (pTrimReductions & currentRule.containsOneNonTerminal()) {
                    ...
                    // and can be trimmed from the
                    // parse tree. Usually we create a new Reduction, assign it
                    // to the Data property
                    // of Head and push it on the stack. However, in this case,
                    // the Data property of the
                    // Head will be assigned the Data property of the reduced
                    // token (i.e. the only one
                    // on the stack).
                    // In this case, to save code, the value popped of the stack
                    // is changed into the head.
                ...
                // ========= If n is -1 here, then we have an Internal Table
                // Error!!!!
                lrAct = (LRActionTable) pActionTable.elementAt(index);
                n = lrAct.actionIndexForSymbol(
                        currentRule.getRuleNonTerminal().getTableIndex());
    ...
    // this method is not accessible. It will throw an ParserException if there
    // was
    // a problem reading the source file.
    private Token retrieveToken(LookAheadStream source) throws ParserException {
        // This function implements the DFA algorithm and returns a token to the
        // LALR state
        // machine

        String ch; // Temporary storage for a character read by the DFA
                   // algorithm

        int n, currentDFA;

        boolean found; // Used to exit a branch search loop. For each state in
                       // the DFA
                       // there are a finite number of branches to other states.
                       // The code
                       // under "Move to next state" searches the branches until
                       // one is
                       // found containing the character (ch).

        boolean done = false; // Used in the main loop. When the next character
                              // is not
                              // found in the DFA, this variable is set true and
                              // the loop
                              // exits. In addition, based on whether a matching
                              // token
                              // could be found, the code either creates an
                              // error token or
                              // returns the new one.

        int target = 0; // Temporary storage that used to hold the index of the
                        // next state in
                        // the DFA. In other words, the state we are now moving
                        // to.

        int charSetIndex; // Temporary storeage that is used to hold the index
                          // in the
                          // Character Set. This isn't really necessary, but it
                          // does save
                          // code, and thus, sanity! :-)

        int currentPosition; // This is the current read-ahead position on the
                             // source string.
                             // The DFA algorithm uses lookahead logic to
                             // analyze the souce
                             // string. Basically, I set the value of the value
                             // initially
                             // to 1 since we are looking at the next character
                             // in the
                             // input. Each time a character is matched in the
                             // algorithm
                             // and the DFA State advances, so too does the
                             // position in the
                             // source input.

        int lastAcceptState; // Whenever a state is found that accepts a token,
                             // it is marked.
                             // Essentially this follows the algorithms desire
                             // to find the longest
                             // matching token possible.

        int lastAcceptPosition; // This is set at the same time as the
                                // LastAcceptState to
                                // store the read-ahead position in the source
                                // that contains the
        ...
        if (!source.done()) {
            while (!done) {
                // ======= This code searches all the branches of the current
                // DFA state for the next
                // ======= character in the input stream. If found the target
                // state is returned.
                // ======= The InStr() function searches the string
                // pCharacterSetTable.Member(CharSetIndex)
                // ======= starting at position 1 for ch. The pCompareMode
                ...
                if (ch.equals("")) // End reached, do not match
                {
                    ...
                    while ((n < faTmp.edgeCount()) & (!found)) {
                        charSetIndex = Integer
                                .parseInt(faTmp.edge(n).getChars());
                        String sT = (String) pCharacterSetTable
                                .elementAt(charSetIndex);
                        ...
                        for (int i = 0; i < sT.length(); i++) {
                            ...
                            if (k == 10) {
                                ...
                                if (x == k) {
                                    ...
                                    // after that is whitespace, if it is not
                                    // then
                                    // we don't want to include it in our
                                    // whitespace
                                    // token!
                ...
                // ======= This block-if statement checks whether an edge was
                // found from the current state.
                // ======= If so, the state and current position advance.
                // Otherwise it is time to exit the main loop
                // ======= and report the token found (if there was it fact
                // one). If the LastAcceptState is -1,
                // ======= then we never found a match and the Error Token is
                // created. Otherwise, a new token
                // ======= is created using the Symbol in the Accept State and
                // all the characters that
                // ======= comprise it.

                if (found) {
                    // ======= This code checks whether the target state accepts
                    // a token. If so, it sets the
                    // ======= appropiate variables so when the algorithm in
                    // done, it can return the proper
                ...
                } else // No edge found
                {
                    ...
                    if (lastAcceptState == -1) // Tokenizer cannot recognize
                                               // symbol
                    {
                        if (pCommentLevel == 0) {
                        ...
                        FAState faTmp3 = (FAState) pDFA
                                .elementAt(lastAcceptState);
                        result.setParentSymbol(
                                pSymbolTable.getMember(faTmp3.acceptSymbol));
                        result.setData(source.read(lastAcceptPosition));
                    }
                }
            }
        } else {
            result.setData(new String("")); // End of file reached, create End
                                            // Token
